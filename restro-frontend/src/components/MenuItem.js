...
const MenuItemForm = ({ initialValues, handleFormSubmit }) => {
  ...
  const onSubmit = (values, { setSubmitting, resetForm }) => {
    console.log("Form submitted with values:", values); // Debugging form submission

    if (handleFormSubmit) {
      handleFormSubmit(values); // Pass the single menu item to the parent
    }

    // Reset the form to allow adding another item after submission
    resetForm();
